<template>
  <div class="container pl-0 pr-0">
    <nav class="navbar navbar-expand-lg navbar-light">
      <a class="navbar-brand" href="#"> <strong>Demo</strong>store </a>
      <button
        ...
        <ul class="navbar-nav">
          <li v-for="category in categories" :key="category.mediaId" class="nav-item dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
              <a v-for="subcategory in category.children" :key="subcategory.id" class="dropdown-item" href="#">{{ subcategory.name }}</a>
            </div>
          </li>
        </ul>
      </div>
